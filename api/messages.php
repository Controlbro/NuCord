...
$stmt = db()->prepare('SELECT m.id,m.body,m.media_path,m.media_type,m.media_name,m.created_at,u.username,u.id user_id FROM messages m JOIN users u ON u.id=m.user_id WHERE m.conversation_id=? AND m.id>? ORDER BY m.id ASC LIMIT 100');
