...
if($action==='add_members'){
    ...
    if(!is_owner($pdo,$cid,(int)$user['id'])) json_response(['ok'=>false,'error'=>'Owner only.'],403);
    $ids=array_values(array_unique(array_map('intval',$_POST['members']??[])));
    $count=$pdo->prepare('SELECT COUNT(*) FROM conversation_members WHERE conversation_id=?'); $count->execute([$cid]);
    $space=max(0,21-(int)$count->fetchColumn());
    if($space<1) json_response(['ok'=>false,'error'=>'This group is already at the 21 person limit.'],422);
    $ids=array_slice($ids,0,$space); $added=0;
    $friend=$pdo->prepare('SELECT 1 FROM friends WHERE user_id=? AND friend_id=?');
    $ins=$pdo->prepare('INSERT IGNORE INTO conversation_members (conversation_id,user_id) VALUES (?,?)');
    foreach($ids as $id){ if($id===(int)$user['id']) continue; $friend->execute([$user['id'],$id]); if($friend->fetchColumn()){ $ins->execute([$cid,$id]); $added += $ins->rowCount()>0 ? 1 : 0; } }
