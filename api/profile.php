...
if($action==='status'){ $status=($_POST['status']??'online')==='dnd'?'dnd':'online'; $pdo->prepare('UPDATE users SET status=? WHERE id=?')->execute([$status,$user['id']]); json_response(['ok'=>true,'status'=>$status]); }
