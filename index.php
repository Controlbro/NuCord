...
<aside class="sidebar">
  ...
  <div class="me"><img class="avatar-img" src="<?= h(asset_url($user['avatar'])) ?>" alt=""><div class="me-name"><b><?= h($user['username']) ?></b><small><span class="status-dot <?= h($user['status']) ?>"></span><?= $user['status']==='dnd'?'Do Not Disturb':($user['status']==='offline'?'Offline':'Online') ?></small></div><select id="statusSelect"><option value="online" <?= $user['status']==='online'?'selected':'' ?>>Online</option><option value="dnd" <?= $user['status']==='dnd'?'selected':'' ?>>Do Not Disturb</option><option value="offline" <?= $user['status']==='offline'?'selected':'' ?>>Offline</option></select><label class="mini-upload">Avatar<input id="avatarUpload" type="file" accept="image/*" hidden></label><a class="mini-logout" href="logout.php">Logout</a></div>
</aside>
...
<aside id="memberPane" class="member-pane"><h3>Members</h3><div id="memberList"></div></aside><div id="contextMenu" class="context-menu" hidden></div><div id="groupModal" class="modal" hidden></div>
