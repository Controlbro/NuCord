...
$user = require_login();
$pdo = db();
$friends = $pdo->prepare('SELECT u.id, u.username FROM friends f JOIN users u ON u.id=f.friend_id WHERE f.user_id=? ORDER BY u.username');
...
<section><h3>Friends</h3><form id="searchForm" class="search"><input name="q" placeholder="Search username"><button>Find</button></form><div id="friendResults"></div><div id="requests"></div><ul id="friendsList"><?php foreach ($friends as $f): ?><li><button class="friend" data-id="<?= (int)$f['id'] ?>">@<?= h($f['username']) ?></button></li><?php endforeach; ?></ul></section>
...
<main class="chat"><header class="chat-top"><div><h2 id="chatTitle">Select a conversation</h2><p id="chatSubtitle">Start a DM from your friends list.</p></div></header><div id="messages" class="messages"></div><div id="typing" class="typing"></div><form id="messageForm" class="composer"><textarea id="messageInput" placeholder="Message NuCord" rows="1"></textarea><button>Send</button></form></main>
